...
public class ShoppingCartServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    boolean isLoginNeeded = false; // Triggers a forced login on exiting the cart, if
    ...
    // Ensure we have a valid cart -- use session variable, or make one if it doesn't
    // exist yet
    ShoppingCart cart =
    ...
    if (request.getParameter(props.getProperty("SC_UPDATE")) != null) {
       ...
       * itemName[idx]: <String> itemQuantity[idx]: <Integer>
       * 
       * these indexes can be parsed out using substring() and correspond to the
       * indices that the shopping cart java object maintains, that is itemName0 and
       * itemQuantity0 correspond to the shopping cart item returned by
       * ShoppingCart#getItem(0). Furthermore, we have exactly one pair of these
       * name/quantities for every current object in the shopping cart. Take careful
       * note that updating a quantity to zero (0) should really remove it from the
       * cart, and the resulting cart listing sent back will contain one less item
       * than it did before for every quantity that was set to zero (0).
       */
      try {
        List<ShoppingCartItem> cartItems = cart.getCartContents();
        for (int idx = 0; idx < cartItems.size(); idx++) {
          String qtyAsString =
              request.getParameter(props.getProperty("SC_ITEM_QUANTITY") + idx);
      ...
      /*
       * Steps involved in checking out the cart: 1. Wrap the carts contents using
       * ShoppingCartWrapper. 2. Marshall XML file for the PO to some predetermined
       * folder. 3. (for now) : set target so that the user can view their link to
       * the PO
       */
      CustomerBean customer =
          (CustomerBean) session
              .getAttribute(props.getProperty("INTERNAL_CUSTOMER"));
      ...
      if (customer == null) {
        ...
        try {
          String rootFolder =
              ctx.getRealPath(props.getProperty("SC_PO_ROOT_FOLDER"));
          ...
          ShoppingCartWrapper wrapper =
              new ShoppingCartWrapper(customer, orderId, cart.getCartContents());
        ...
        // If session is untouched for 5 mins, count as fresh visit -- should be
        // placed some place more general
        session.setMaxInactiveInterval(300);
      }
      /* FOR AJAX */
    } else if ((request.getParameter("addName") != null)
      ...
      try {
        ...
        request.setAttribute("errorCart", "Item " + itemID
            + " does not exist or is no longer available!");
        out.write("Server says: add failed\nNo such item:" + itemName);
      }
    }

    // Update the cart prices on whatever is left in the cart. MUST be done to show
    // the user the most
    ...
    if (isLoginNeeded) {
      ...
      if (isPurchaseOrderCreated) {
        ...
        request.setAttribute(props.getProperty("CART_CONTENTS"),
